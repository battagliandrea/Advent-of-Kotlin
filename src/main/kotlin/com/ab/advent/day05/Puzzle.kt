...
import com.ab.advent.utils.readText
...
a quantity of crates is moved from one stack to a different stack. In the first step of the above rearrangement procedure, one crate is moved from stack 2 to stack 1, resulting in this configuration:
...
fun main() {
    ...
    println(answer1("puzzle_05_input.txt".readText()))
    println(answer2("puzzle_05_input.txt".readText()))
}
...
fun answer1(input: String) = Parser.parse(input)
    .let { (stacks, movements) -> Crane(stacks(), crateMover9000).apply { move(movements) }.message }

fun answer2(input: String) = Parser.parse(input)
    .let { (stacks, movements) -> Crane(stacks(), crateMover9001).apply { move(movements) }.message }
